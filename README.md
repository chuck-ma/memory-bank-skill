...
当收到系统提醒时，用户可以选择"更新并提交"或"初始化并提交"，将 Memory Bank 更新和代码变更一起提交到 Git。

**操作选项**：

| 用户回复 | 行为 |
|---------|------|
| "初始化" / "更新" | 只更新 Memory Bank，不提交 |
| "初始化并提交" / "更新并提交" | 更新 Memory Bank + `git add -A` + `git commit` |
| "跳过" / "跳过初始化" | 本次会话不再提醒 |

**智能跳过**：

当插件通过 `git status` 检测到 `memory-bank/` 目录下有未提交的变更时，会自动标记为"已更新"，避免重复提醒。这意味着：
- 只要你更新了 memory-bank 文件（无论是否提交），当前会话不会再提醒
- 提交后 git 状态干净，下次会话也不会再提醒同样的内容
...
**INIT 提醒**（初始化）：
- 检测到项目**没有** `memory-bank/` 目录时**自动触发**
- 不需要文件修改或关键词，只要目录不存在就提醒

**UPDATE 提醒**（更新）：
- 项目**已有** `memory-bank/` 目录时，满足以下任一条件触发：
  1. **文件修改**：修改了 1 个或更多代码/配置/文档文件
  2. **关键词检测**：用户消息包含上述关键词

**设计理念**：

- INIT 提醒确保每个项目都能及时初始化 Memory Bank
- UPDATE 提醒确保重要变更被记录
- 用户可回复"跳过"/"跳过初始化"/"无需更新"跳过提醒
...
- **版本**: 4.1.0
- **主要更新**:
  - **自动提交模式**：用户可选择"更新并提交"将 Memory Bank 更新和代码变更一起提交
  - **自动初始化提醒**：检测到项目无 `memory-bank/` 目录时自动提醒初始化
